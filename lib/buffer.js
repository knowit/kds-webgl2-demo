...
export const framebuffer = ({ gl }, width, height, type = textureTypes.FLOAT) => {
    ...
    return {
        ...
        read: (buffer) => {
            buffer = buffer || (type.type === gl.FLOAT ? new Float32Array(width*height) : new Uint8Array(width*height))
            gl.readBuffer(0, 0, width, height, type.format, type.type, buffer)
            return buffer
        }
    }
}
